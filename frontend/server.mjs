...
const openBrowserOnStart = process.env.OPEN_BROWSER !== 'false' && !process.argv.includes('--no-open');
...
const server = http.createServer(async (req, res) => {
  const url = new URL(req.url || '/', `http://${req.headers.host || `localhost:${requestedPort}`}`);
...
function listen(port, attemptsLeft = 10) {
  ...
  server.once('error', error => {
    if ((error.code === 'EADDRINUSE' || error.code === 'EACCES') && attemptsLeft > 0) {
      ...
      console.warn(`Port ${port} is not available (${error.code}), trying ${nextPort}...`);
  ...
  server.listen(port, host, () => {
    ...
    console.log(`  Health:  ${localUrl}/api/health/detail`);
    console.log(`  Demo account: admin / 123456\n`);
    if (openBrowserOnStart) openBrowser(localUrl);
  });
}
