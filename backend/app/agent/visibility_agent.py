...
def analyze_frame(frame, detections: list[dict] | None = None) -> dict:
    ...
    evidence = []
    direct_visibility_score = 0
    quality_score = 0

    if brightness < 55:
        direct_visibility_score += 2
        evidence.append("画面亮度较低，疑似夜间、低光或曝光不足场景")
    elif brightness < 85:
        direct_visibility_score += 1
    ...
    visibility_score = _combine_visibility_score(direct_visibility_score, quality_score)
    ...
    return {
        ...
        "visibility_risk": _risk_name(visibility_score),
        "visibility_score": visibility_score,
        "visual_quality_score": min(3, quality_score),
...
def aggregate_visibility(samples: list[dict]) -> dict:
    ...
    quality_score = mean(sample.get("visual_quality_score", 0) for sample in samples)
    ...
    return {
        ...
        "visual_quality_score": round(quality_score, 2),
        "scene_quality": _scene_quality(brightness, contrast, sharpness),
...
def _combine_visibility_score(direct_visibility_score: int, quality_score: int) -> int:
    """Keep camera/model quality issues from becoming high weather risk alone."""
    if direct_visibility_score <= 0:
        return 1 if quality_score >= 2 else 0
    if direct_visibility_score >= 2 and quality_score >= 2:
        return min(3, direct_visibility_score + 1)
    return min(3, direct_visibility_score)
